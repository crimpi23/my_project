...
def add_to_cart():
    try:
        ...
        print(f"Received data: article={article}, price={price}, quantity={quantity}, table_name={table_name}")

        # Перетворюємо типи
        if not (article and price and quantity and table_name):
        ...
        if not product:
            print(f"Product not found in database: article={article}, price={price}, table_name={table_name}")
            return render_template('index.html', message="Product not found in database.")

        product_id = product['id']

        # Перевірка існування продукту в кошику
        cursor.execute("""
            SELECT id FROM cart
        ...
        if existing_cart_item:
            ...
            print(f"Updated quantity for product_id={product_id} in cart.")
        else:
            ...
            print(f"Inserted new product_id={product_id} into cart.")
